...
public:
  ...
  void AddCli(int _fd, FdType _type){
    FD _id; 
    _id.fd=_fd; 
    _id.type=_type;
    cli.push_back(_id);
  ...
  void AddUnix(const char *_path){
    fd=socket(PF_UNIX, SOCK_STREAM, 0);
    struct sockaddr_un sun;
    bzero(&sun,sizeof(sun));
    sun.sun_family=AF_UNIX;
    strcpy(sun.sun_path,_path);
    unlink(_path);
    bind(fd,(struct sockaddr *)&sun,sizeof(sun));
    listen(fd, 5);
    Add(fd, Unix);
  }
  void parseArgs(char *_buf){
    Args a(_buf, " \t\n\0"); 
    //a.Info();
    if(a.argv[0]){
...
#define MAXSOCKETREADBUFFERSIZE 128
  ...
  void Expire(int _sec=2, int _usec=500000){
    tv.tv_sec=_sec; 
    tv.tv_usec=_usec;
  }
  void Info(void){
    cout << "====================" << endl;
    cout << "Socket Info.: " << endl; 
    cout << "  " << ts.NowStr() << endl;
    cout << "  max: " << max << endl;
