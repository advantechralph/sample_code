...
void timerTest(void){
  ...
  createRTTimer(SIGRTMIN+1, 0, 100*1000000);
  dumpNow();
  char c=0; 
  while(1){
    c=getchar();
...
static void *PoolChild(void *data){
  int shmid=0; 
  key_t key=5678;
// change malloc to shared memory instead
#if 0
  pPoolTab=malloc(sizeof(Pool)*PoolTabSize);
#else
  if ((shmid = shmget(key, sizeof(Pool)*PoolTabSize, IPC_CREAT | 0666)) < 0) {
    perror("shmget");
    exit(1);
  }
  if ((pPoolTab = shmat(shmid, NULL, 0)) == NULL) {
    perror("shmat");
    exit(1);
  }
#endif
...
#define timer2Mod 100
static void timer2(int s){
  if((timer2Count%timer2Mod)==(timer2Mod-1)){
    dumpNow();
    timer2Count=0; 
  }else{
    timer2Count++; 
